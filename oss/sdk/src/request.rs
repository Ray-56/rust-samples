...
impl RequestBuilder {
    ...
    pub fn response_content_disposition<S: AsRef<str>>(mut self, file_name: S) -> Self {
        self.oss_headers.insert("response-content-disposition".to_string(), format!("attachment;filename={}", file_name.as_ref()));
        self
    }
    pub fn oss_signature_version2(mut self) -> Self {
        self.oss_headers.insert("x-oss-signatureversion".to_string(), "OSS2".to_string());
        self
    }
    pub fn response_content_encodeing<S: AsRef<str>>(mut self, encodeing: S) -> Self {
        self.oss_headers.insert("response-content-encoding".to_string(), encodeing.as_ref().to_string());
        self
    }
      ...
      where IP: AsRef<str>, S: Into<u8>
    {
      ...
      self.parameters.insert("x-oss-ac-allow-ip".to_string(), mask.into().to_string());
      self
    }
