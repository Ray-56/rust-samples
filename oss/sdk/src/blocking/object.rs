...
impl OSS {
    ...
    pub fn get_upload_object_policy(&self, build: PolicyBuilder) -> Result<PolicyResp, OssError> {
        let date = chrono::Local::now().naive_local()
            + chrono::Duration::try_seconds(build.expire).unwrap();
        ...
        json_data = json_data.replacen("{prefix}", &build.upload_dir, 1);
    ...
    pub fn put_object_from_file<S: AsRef<str>>(
        &self,
        key: S,
        file_path: S,
        build: RequestBuilder,
        ...
        let client = reqwest::blocking::Client::new();
        let response = client.put(url).headers(headers).body(buffer).send()?;
        return if response.status().is_success() {
            Ok(())
        } else {
            let status = response.status();
            let result = response.text()?;
            debug!("put object status: {} error: {}", status, result);
            Err(OssError::Err(format!(
                "put object status: {} error: {}",
                status, result
            )))
        };
    }

    pub fn put_object_from_buffer<S: AsRef<str>>(
        &self,
        key: S,
        buffer: &[u8],
        build: RequestBuilder,
    ...
    pub fn delete_object<S: AsRef<str>>(
        &self,
        key: S,
        build: RequestBuilder,
    ) -> Result<(), OssError> {
        ...
        let (url, headers) = self
            .build_request(key.as_str(), build)
            .map_err(|e| OssError::Err(format!("build request error: {}", e)))?;
        debug!("delete object url: {} headers: {:?}", url, headers);
        let client = reqwest::blocking::Client::new();
        ...
        return if response.status().is_success() {
            ...
            let status = response.status();
            let result = response.text()?;
            debug!("delete object status: {} error: {}", status, result);
            Err(OssError::Err(format!(
	...
	pub fn get_object_metadata<S: AsRef<str>>(
		&self,
		key: S,
		build: RequestBuilder,
		...
		let (url, headers) = self
			.build_request(key.as_str(), build)
			.map_err(|e| OssError::Err(format!("build request error: {}", e)))?;
		debug!("get object metadata url: {} headers: {:?}", url, headers);
		let client = reqwest::blocking::Client::new();
		let response = client.head(url).headers(headers).send()?;
		return if response.status().is_success() {
			...
			let status = response.status();
			let result = response.text()?;
			debug!("get object metadata status: {} error: {}", status, result);
			Err(OssError::Err(format!(
